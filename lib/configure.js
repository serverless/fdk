    ...
    .then(functions =>
      Promise.all(createSubscriptions(params))
        ...
        .catch(subscriptionError => {
          throw new Error(
            `Failed to configure the Event Gateway with the error: ${subscriptionError.message}`
          )
        })
    )
    .catch(functionError => {
      throw new Error(
        `Failed to configure the Event Gateway with the error: ${functionError.message}`
      )
    })
}
